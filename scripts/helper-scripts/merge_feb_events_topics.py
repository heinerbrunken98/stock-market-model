...
# ---------- Filter to February 2018 ----------
start, end = pd.Timestamp("2018-02-01"), pd.Timestamp("2018-02-28")
events = events[(events["day"] >= start) & (events["day"] <= end)].copy()
topics = topics[(topics["day"] >= start) & (topics["day"] <= end)].copy()

# Keep only Top-5 topics per day (but without rank column)
...
events_topics.to_csv(out_dir / "events_topics_top5_feb2018.csv", index=False)
...
freq_event.to_csv(out_dir / "topics_frequency_event_days_top5_feb2018.csv", index=False)
freq_rest.to_csv(out_dir / "topics_frequency_rest_days_top5_feb2018.csv",  index=False)

# ---------- Print quick heads ----------
print("Saved:", out_dir / "events_topics_top5_feb2018.csv")
print("Saved:", out_dir / "topics_frequency_event_days_top5_feb2018.csv")
print("Saved:", out_dir / "topics_frequency_rest_days_top5_feb2018.csv")
print("\nEvent-matched Top-5 topics (Feb 2018, first rows):")
print(events_topics.head(15))
print("\nEvent-day frequency (Feb 2018, Top-5 only):")
print(freq_event.head(15))
print("\nRest-day frequency (Feb 2018, Top-5 only):")
print(freq_rest.head(15))
